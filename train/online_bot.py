# -*- coding: utf-8 -*-
"""曲靖飞小鸡线上机器人：从 vendor/qjfxj 的 Lua AI 扒下来的出牌/鸣牌。"""
...
def _remain_bytes(hand, rivers, melds):
    seen = Counter(hand)
    for seat in rivers:
        seen.update(rivers[seat])
        seen.update(melds[seat])
    out = {}
    for tid in TILE_IDS:
...
def pick_online_discard(hand, rivers, melds, forbid=None):
...
def pick_online_call(hand, offer, offer_seat, seat, own_turn, n_melds, mask, wall_n):
    if mask[CALL_HU]:
        return CALL_HU
    if own_turn:
...
class OnlineBots:
    def pick_discard(self, seat, dealer, hands, rivers, melds, n_melds, just_drew, forbid=None, **kwargs):
        return pick_online_discard(hands[seat], rivers, melds, forbid=forbid)
    ...
    def pick_call(self, seat, dealer, hands, rivers, melds, n_melds, offer, offer_seat, own_turn, just_drew, wall_n, **kwargs):
        mask = legal_call_mask(
            ...
            after_gang=kwargs.get("after_gang", False), melds=melds[seat] if seat in melds else None,
        )
        if wall_n <= 0:
            mask[CALL_AN_GANG] = 0
        ...
        if sum(mask[1:]) == 0:
            return CALL_PASS
        return pick_online_call(
            hands[seat], offer, offer_seat, seat, own_turn, n_melds[seat], mask, wall_n,
        )
...
class MixedBots:
    def __init__(self, kinds):
        ...
        from train.imitation import load_imitation_bots
        self.model = load_imitation_bots() if "model" in kinds.values() else None

    def pick_discard(self, *args, **kwargs):
        seat = args[0]
        if self.kinds[seat] == "online":
            return self.online.pick_discard(*args, **kwargs)
        return self.model.pick_discard(*args, **kwargs)

    def pick_call(self, *args, **kwargs):
        seat = args[0]
        if self.kinds[seat] == "online":
            return self.online.pick_call(*args, **kwargs)
        return self.model.pick_call(*args, **kwargs)
